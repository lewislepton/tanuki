...
import tanuki.math.Vector4D;

/**
  A 4x4 matrix
 */
abstract Matrix4D(Matrix4) from Matrix4 to Matrix4  {
    ...
    /**
      ...
      @return Matrix4D
     */
    public inline static function identity(dest:Matrix4D):Matrix4D {
        dest.r0c0 = 1;
        dest.r0c1 = 0;
        dest.r0c2 = 0;
        dest.r0c3 = 0;
    ...
    /**
      ...
      @return Matrix4D
     */
    public inline static function copy(src:Matrix4D, dest:Matrix4D):Matrix4D {
        dest.r0c0 = src.r0c0;
        dest.r0c1 = src.r0c1;
        dest.r0c2 = src.r0c2;
        dest.r0c3 = src.r0c3;
    ...
    /**
      ...
      @return Matrix4D
     */
    public inline static function transpose(src:Matrix4D, dest:Matrix4D):Matrix4D {
    ...
    /**
      ...
      @return Float
     */
    public inline static function determinant(src:Matrix4D):Float {
    ...
    /**
      ...
      @return Matrix4D
     */
    public inline static function invert(src:Matrix4D, dest:Matrix4D):Matrix4D {
    ...
    /**
      ...
      @return Matrix4D
     */
    public inline static function multMat(a:Matrix4D, b:Matrix4D, dest:Matrix4D):Matrix4D {
        // cache what we need to do the calculations
        var _a:Matrix4D;
        var _b:Matrix4D;
        if(dest == a) {
            _a = Matrix4D.copy(a, new Matrix4D());
            _b = b;
        }
        else if(dest == b) {
            _a = a;
            _b = Matrix4D.copy(b, new Matrix4D());
        }
        else {
            _a = a;
            _b = b;
    ...
    /**
      Shortcut operator for `multMat(a, b, new Matrix4D())`
      @param a 
      @param b 
      @return Matrix4D
     */
    @:op(A * B)
    inline static function multMatOp(a:Matrix4D, b:Matrix4D):Matrix4D {
        return multMat(a, b, new Matrix4D());
    }
    ...
    /**
      ...
      @return Vector4D
     */
    public inline static function multVec(m:Matrix4D, v:Vector4D, dest:Vector4D):Vector4D {
    ...
    /**
      Shortcut for `multVec(m, v, new Vector4D())`
      @param m 
      @param v 
      @return Vector4D
     */
    @:op(A * B)
    inline static function multVecOp(m:Matrix4D, v:Vector4D):Vector4D {
        return multVec(m, v, new Vector4D());
    }

    /**
      Construct a Matrix4D from an array of floats in column-major order
      @param arr an array with 16 elements
      @return Matrix4D
     */
    @:from
    public inline static function fromFloatArray(arr:Array<Float>):Matrix4D {
        return new Matrix4D(
