...
#else
import tanuki.tool.Destroy.IDestroy;
...
abstract TypedSignal<T>(ISignal<T>)
{
	...
	@:to 
	static inline function toSignal0(signal:ISignal<Void->Void>):FlxSignal0 
	{
		return new FlxSignal0();
	}
	
	@:to 
	static inline function toSignal1<T1>(signal:ISignal<T1->Void>):FlxSignal1<T1> 
	{
		return new FlxSignal1();
	}
	
	@:to 
	static inline function toSignal2<T1, T2>(signal:ISignal<T1->T2->Void>):FlxSignal2<T1, T2> 
	{
		return new FlxSignal2();
	}
	
	@:to 
	static inline function toSignal3<T1, T2, T3>(signal:ISignal<T1->T2->T3->Void>):FlxSignal3<T1, T2, T3> 
	{
...
private class FlxSignalHandler<T> implements IDestroy
{
...
private class FlxBaseSignal<T> implements ISignal<T> 
{
	/**
...
private class Signal4<T1, T2, T3, T4> extends FlxBaseSignal<T1->T2->T3->T4->Void>
{
	public function new()
	{
		super();
...
interface ISignal<T> extends IDestroy
{
	var dispatch:T;
